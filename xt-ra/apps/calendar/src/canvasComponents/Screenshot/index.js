...
class Screenshot extends Component {
  ...
  render() {
    ...
    const effectImg = productType ? `./assets/${productType}/${productSize}/${upgradeCover}.png` : '';
      ...
      if (project.pageArray) {
        project.pageArray.forEach((page, index) => {
          const pageData = {
            ...
            project,
            capability,
            x: get(size, 'renderCoverSheetSizeWithoutBleed.left') + get(size, 'renderCoverSheetSize.left')+2,
            y: get(size, 'renderCoverSheetSizeWithoutBleed.top') + get(size, 'renderCoverSheetSize.top'),
            isUpgradeTo8X6,
            userInfo
          };
