...
class BookPageThumbnail extends Component {
  ...
  onBookPageDrop() {
    const { settings,page } = this.props.data;
    const { reApplyDefaultTemplateToPage } = this.props.actions;
    const { elementArray } = this.state;
    const product = settings.spec.product;
    const hasPhotoElement = elementArray.find(
      element => element.get('type') === elementTypes.photo
    );
    const isPEnecessary = !(
      product === productTypes.WC && page.get('type') ===  pageTypes.bottom
    );
    if (isPEnecessary && !hasPhotoElement) {
  ...
  render() {
    ...
    return (
      <div
        ...
        style={bookPageStyle}
        onDrop={this.onBookPageDrop.bind(this)}
      >
        {this.getRenderHtml()}
      </div>
    );
