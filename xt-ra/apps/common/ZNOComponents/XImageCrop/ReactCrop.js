/**
 * 修改日志:
 *  - 禹赛(2017/3/29): 基于v1.0.3版本, 添加了onImageError回调.
 */
...
class ReactCrop extends Component {
  ...
  onImageLoad(imageEl) {
    ...
    if(imageEl){
      this.setState({imageWidth: imageEl.naturalWidth,imageHeight: imageEl.naturalHeight})
    }
  ...
  ensureAspectDimensions(cropObj, imageEl) {
    ...
    const imageAspect = imageWidth ? (imageWidth / imageHeight) : 1;
  ...
  renderSvg() {
    ...
    return (
      <svg width={this.state.imageWidth} height={this.state.imageHeight} style={{ position: 'absolute' }}>
        <defs>
