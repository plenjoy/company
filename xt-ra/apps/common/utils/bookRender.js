...
export const run = (imgObjArray, done) => {
  ...
  imgObjArray.forEach((item, i) => {
    combine(item.images, Math.ceil(item.size.width), Math.ceil(item.size.height), item.paddings, item.outPaddings, (opt) => {
      imgObj[i] = opt;
      callback();
    });
  });
