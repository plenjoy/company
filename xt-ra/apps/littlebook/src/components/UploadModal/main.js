...
const uploadTracker = (that, successUploaded, errorUploaded) => {
  ...
  timer = setTimeout(() => {
    ...
    const totalUploadedImages = project.get('imageArray').count();
  ...
  if (newImages) {
    ...
    newImages.forEach((file, index) => {
      ...
      const reducer = instance.push({
        ...
        error: err => {
          const { countOfFailed, countOfCompleted } = instance;
          const endTime = Date.now();
...
export const handleUploadModalClosed = (that, isManuClick) => {
  ...
  // 获取所有照片的列表.
  const uploadedImages = that.props.project.get('imageArray');
  ...
  if (!checkIsAllCompleted() && isManuClick) {
    ...
    boundConfirmModalActions.hideConfirm();
    // 当项目图片大于24的时候才关闭
    if (uploadedImages.count() >= leastUploadImagesCount || isManuClick) {
      closeUploadedImagesModal();
    }
  }
};
