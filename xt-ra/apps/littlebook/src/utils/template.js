...
import {
  limitImageNumberInHalfPageTemplate,
  pageTypes,
  layoutSheetType,
  coverTypes
  ...
  if (data && data.length) {
    data.forEach((t) => {
      const temp = merge(
        ...
        {
          id: t.id,
          template: convertObjIn(x2jsInstance.xml2js(t.template))
        }
      );
 ...
 */
export const formatTemplateInstance = (results, templateIds) => {
  const newResults = [];
  ...
  if (templateIds && templateIds.length) {
    templateIds.forEach((id) => {
      ...
      if (template) {
        const elements = convertObjIn(
          get(template, 'template.templateView.spread.elements.element')
        );
        ...
        if (doValid(newElements)) {
          newResults.push({
            id: template.id,
            bgColor: numberToHex(
              get(template, 'template.templateView.spread.bgColor')
            ),
...
export const filterCoverTemplates = (templates, coverType) => {
  ...
  if (isSupportFullImageInCover) {
    ...
    newList = templates.filter((template) => {
      return (
        template.sheetType.toLowerCase() === 'hc' &&
        template.pageType.toLowerCase() === 'full' &&
        (template.customerId !== 0 ||
          (template.imageNum > 0 &&
            template.imageNum <= limitImageNumberInHalfPageTemplate))
      );
    });
    ...
    newList = templates.filter((template) => {
      return (
        template.sheetType.toLowerCase() === 'cc' &&
        template.pageType.toLowerCase() === 'full' &&
        (template.customerId !== 0 ||
          (template.imageNum > 0 &&
            template.imageNum <= limitImageNumberInHalfPageTemplate))
...
export const getLayoutTypeByCover = coverType => {
  let type = layoutSheetType.PA;

  switch(coverType){
    case coverTypes.LBHC :{
      type = layoutSheetType.HC;
      break;
    }
    case coverTypes.LBPAC :{
      type = layoutSheetType.PA;
      break;
    }
    default:{
      break;
