...
class BookSheet extends Component {
  ...
  render() {
    ...
    const extraString = isLandscapeShape ? '_landscape' : '';
    const imgName = thumbnail ? `thumbnail_inner${extraString}` : `inner${extraString}`;
    ...
    const innerContainerStyle = merge(
      {},
      {
        width: `${get(size, 'renderInnerSize.width')}px`,
        height: `${get(size, 'renderInnerSize.height')}px`,
        top: `${(parseInt(containerStyle.height) - get(size, 'renderInnerSize.height')) / 2}px`,
        left: `${(parseInt(containerStyle.width) - get(size, 'renderInnerSize.width')) / 2}px`,
        cursor: isNavpages ? 'pointer' : 'default'
      }
    );
    ...
    return (
      ...
      >
        {thumbnail && productType !== productTypes.LPS
