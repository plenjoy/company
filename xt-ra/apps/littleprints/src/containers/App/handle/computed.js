...
export const recomputedPreviewRatios = (props) => {
  ...
  const previewCoverWorkspaceRatio = computedWorkSpaceRatio(size.coverBgParams, {
    top: 100,
    right: 50,
    bottom: 100,
    left: 50
  ...
  const previewInnerWorkspaceRatio = computedWorkSpaceRatio(size.innerBgParams, innerOffset, percent.lg);

  const previewInnerWorkspaceRatioLandscape = computedWorkSpaceRatio(size.innerBgParamsLandscape, innerOffset, percent.lg);
  ...
  boundRatioActions.updateRatio([
    ...
    { type: ratioTypes.previewInnerWorkspaceLandscape, ratio: previewInnerWorkspaceRatioLandscape }
  ]);
};
