...
export const onRect = (that, element, event) => {
  if (element.get('cameoShape') === cameoShapeTypes.rect) return;

  const { data, actions } = that.props;
  const { parameters } = data;
  const { boundProjectActions, boundTrackerActions } = actions;

  boundTrackerActions.addTracker(`ChangeCameoShape, ${element.get('cameoShape')}, ${cameoShapeTypes.rect}`);
...
export const onRound = (that, element, event) => {
  if (element.get('cameoShape') === cameoShapeTypes.round) return;
  ...
  boundTrackerActions.addTracker(`ChangeCameoShape, ${element.get('cameoShape')}, ${cameoShapeTypes.round}`);
 ...
 */
const updateElementBySize = (that, element, cameo) => {
  const { data, actions } = that.props;
  const { boundProjectActions } = actions;
...
export const onSmall = (that, element, event) => {
  if (element.get('cameo') === cameoSizeTypes.small) return;

  const { data, actions } = that.props;
  const { parameters } = data;
  const { boundProjectActions, boundTrackerActions } = actions;

  boundTrackerActions.addTracker(`ChangeCameoSize, ${element.get('cameo')}, ${cameoSizeTypes.small}`);
  ...
  boundProjectActions
    ...
    .then(() => {
      updateElementBySize(that, element, cameoSizeTypes.small);
    });
  stopEvent(event);
};
...
export const onMedium = (that, element, event) => {
  if (element.get('cameo') === cameoSizeTypes.middle) return;

  const { data, actions } = that.props;
  const { parameters } = data;
  const { boundProjectActions, boundTrackerActions } = actions;

  boundTrackerActions.addTracker(`ChangeCameoSize, ${element.get('cameo')}, ${cameoSizeTypes.middle}`);
  ...
  boundProjectActions
    ...
    .then(() => {
      updateElementBySize(that, element, cameoSizeTypes.middle);
    });
  stopEvent(event);
};
...
export const onLarge = (that, element, event) => {
  if (element.get('cameo') === cameoSizeTypes.large) return;

  const { data, actions } = that.props;
  const { parameters } = data;
  const { boundProjectActions, boundTrackerActions } = actions;

  boundTrackerActions.addTracker(`ChangeCameoSize, ${element.get('cameo')}, ${cameoSizeTypes.large}`);
  ...
  boundProjectActions
    ...
    .then(() => {
      updateElementBySize(that, element, cameoSizeTypes.large);
    });
  stopEvent(event);
};
...
export const onClear = (that, element, event) => {
  ...
  if (element.get('encImgId')) {
    boundTrackerActions.addTracker(`DeleteCameoImage, ${element.get('encImgId')}`);
