...
export const doFilter = (that, tag, list) => {
  ...
  const { baseUrls, setting, page, realFilterTag, userInfo } = data;
  ...
  switch (tag) {
    ...
    case MY:
      newList = list.filter((template) => {
        return !!template.customerId && template.customerId === userId;
      });
      break;
    case TEXT:
      newList = list.filter((template) => {
  ...
  // 如果当前filter比元素个数大
  if (!isNaN(tag) && elementSize < parseInt(tag)) {
    matchedList = copyNewList.filter(m => {
        return m.horizontalNum >= horizontalImgNum &&
          m.verticalNum >= verticalImgNum &&
          m.squareNum >= squareImgNum;
      }) || [];
    matchedAddtionalList = copyNewList.filter(m => {
      return !(m.horizontalNum >= horizontalImgNum &&
          m.verticalNum >= verticalImgNum &&
          m.squareNum >= squareImgNum);
    }) || [];
  ...
  if (copyNewList.length) {
    copyNewList = copyNewList.map((item) => {
      const templateUrl = template(TEMPLATE_SRC)({
        ...
        size,
        guid: item.guid,
        randomMumber: formatDate(new Date())
      });
