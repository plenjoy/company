...
const doApplyTemplate = (that, template, nextProps, elementArray) => {
  const { boundProjectActions, settings, ratio, paginationSpread } =
    nextProps || that.props;

  const images = get(that.props, 'allImages');
  const page = paginationSpread.get('page');
  const summary = paginationSpread.get('summary');

  const elements = elementArray || page.get('elements');
  let newPage = page;
  ...
  const textOrPhotoElements = elements;
  ...
  if (textInTemplate.length) {
    ...
    if (backgroundElements && backgroundElements.size) {
      backgroundElements.forEach(ele => {
        iElements.push(ele);
      });
    }
  ...
  if (isEnableAutoLayout && elements.size) {
    ...
    elements.forEach(element => {
      if (
        ...
        const elementImage = getImage(images, element.get('encImgId'));
    ...
    if (guid) {
      ...
      const cloneAddedElements = addedElements.slice(0);

      let countNoImageElement = 0;
      ...
      pageElements.forEach((element, index) => {
        if (
          ...
          countNoImageElement += 1;

          const addedElement = cloneAddedElements.pop();

          newPageElements = newPageElements.setIn(
            [String(index), 'encImgId'],
            addedElement.encImgId
          );
        }
      });

      if (countNoImageElement) {
        const newElements = mergeTemplateElements(
          page.set('elements', newPageElements),
          newPageElements,
          allImages
        );
        ...
        boundProjectActions.applyTemplate(
          ...
          newElements.concat(Immutable.fromJS(cloneAddedElements))
        );
      } else {
