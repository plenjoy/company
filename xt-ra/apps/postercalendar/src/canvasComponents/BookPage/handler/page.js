...
const convertElement = (that, currentPage, curElement, index, x, y) => {
  ...
  const shouldSortElements = elements.filter((item) => {
    return item.get('type') !== elementTypes.calendar;
  });
  const maxDepElement = shouldSortElements.maxBy((item) => {
    return item.get('dep');
  });
...
export const componentWillReceiveProps = (that, nextProps) => {
  ...
  const oldOrientation = get(that.props.data, 'settings.spec.orientation');
  const newOrientation = get(nextProps.data, 'settings.spec.orientation');
