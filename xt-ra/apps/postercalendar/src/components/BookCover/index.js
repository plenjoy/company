...
class BookCover extends Component {
  ...
  render() {
    ...
    const requestSize = orientation === 'Landscape' ? `${productSize.split('X')[1]}X${productSize.split('X')[0]}` : productSize;
    const effectImg = `./assets/${productType}/${requestSize}/${imgName}.png?randow=20171013`;
    ...
    const containerStyle = merge({},
      {},
      {
        ...
        cursor: 'default',
        marginBottom: productType === productTypes.DC
          ? '46px'
          : productType === productTypes.PC
            ? 0
            : '40px'
      },
      styles
