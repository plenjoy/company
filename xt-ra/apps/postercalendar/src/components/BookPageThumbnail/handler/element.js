...
export const computedElementOptions = (that, element, workspaceRatio) => {
  const { data } = that.props;
  const { urls, page, images, settings, userInfo } = data;
  ...
  switch (element.get('type')) {
    case elementTypes.text: {
      if (data.page.get('type') === pageTypes.spine) {
        ...
        const scale = 1 / ratio;
        const color = element.get('fontColor')
      ...
      if (encImgId) {
        ...
        if (cropOptions) {
          ...
          if (element.get('style')) {
            filterOptions = merge(
              {},
              filterOptions,
              element.get('style').toJS()
            );
          }

          obj.imgUrl = template(`${IMAGES_API}${IMAGES_FILTER_PARAMS}`)(
        ...
        if (border) {
          if (!border.get('size')) {
            ...
            obj.border = `${Math.round(
              border.get('size') * ratio
            )}px solid ${rbga}`;
          }
        }
      }
