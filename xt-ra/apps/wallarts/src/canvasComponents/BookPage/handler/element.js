...
import { checkElementTemplateType, templateTypes } from '../../../utils/customeTemplate';
...
export const computedElementOptions = (that, nextProps, element, workspaceRatio) => {
  ...
  const ratio = workspaceRatio || data.ratio;
  const obj = {
    width: 0,
    height: 0,
    left: 0,
    ...
    case elementTypes.text:
      {
        ...
        const fontSizeInPt = getFontSizeInPt(
          fontSizePercent * (isSpine ? (page.get('height') - 300) : page.get('height'))
        );
    ...
    case elementTypes.photo:
      {
        ...
        const bleed = element.get('bleed');

        if (bleed) {
          const bleedLeft = bleed.get('left') * ratio;
          const bleedTop = bleed.get('top') * ratio;
          const bleedBottom = bleed.get('bottom') * ratio;
          const bleedRight = bleed.get('right') * ratio;

          obj.rectX = bleedLeft;
          obj.rectY = bleedTop;
          obj.rectWidth = obj.width - (bleedLeft + bleedRight);
          obj.rectHeight = obj.height - (bleedTop + bleedBottom);
        }
