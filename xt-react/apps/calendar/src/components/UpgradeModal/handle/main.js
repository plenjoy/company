...
export const computedOptionsSize = that => {
  ...
  const wrapWidth = fromProductSize === '5X7' ? 670 : 640;
  ...
  html.push(<UpgradeItem key="to-item" data={toData} actions={toActions}>
    <div className="options">
      ...
      <span className="additional">
        {getPrice(basePrice, currentCheckedItems)}
      </span>
    </div>
    <div className="description">
      {getDescription(currentCheckedItems, t)}
    </div>
  </UpgradeItem>);
...
export const updateArrowMarginBottom = (that, size) => {
  if (size) {
    that.setState({
      arrowMarginBottom: (size.height - 14) / 2
    });
  }
};
