...
class App extends Component {
  constructor(props) {
    ...
    this.saveProjectWithOutScreenShot = (
      onSaveSuccessed,
      onSaveFailed,
      isFirstTime,
      isShowNotification
  ...
  componentDidUpdate(prevProps) {
    ...
    if (oldSize !== newSize && customerId !== -1) {
      boundTemplateActions
        .getTemplateList(customerId, newSize, templateTypes[produtType])
        .then(
          a => {},
          err => {
            boundTrackerActions.addTracker('templateListLoadFail');
          }
      );
    }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isNewProject) {
      ...
      const productType = get(nextProps, 'settings.spec.product');
      const shouLoadMainProjectImages =
        productType === productTypes.LC
          ? newIsInnerDefaultTemplateUsed &&
            oldIsInnerDefaultTemplateUsed !== newIsInnerDefaultTemplateUsed
      ...
      if (isCrossProject && shouLoadMainProjectImages) {
        this.loadMainProjectImages();
      }
      // 加载myphoto的图片
      if (shouLoadMainProjectImages && urlQs.get('isFromMyPhoto')) {
      ...
      // MyPhotos 过后需要保存一次项目。
      const newShouldSaveProjectAfterMyPhotos = newProject.property.get(
        'shouldSaveProjectAfterMyPhotos'
      );
