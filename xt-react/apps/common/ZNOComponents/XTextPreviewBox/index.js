...
class XTextPreviewBox extends Component {
  constructor(props) {
    super(props);
  ...
  onLoad(e) {
    const { maxImgContainerWidth, maxImgContainerHeight } = this.props;
    const imageWidth = e.target.naturalWidth;
    const imageHeight = e.target.naturalHeight;

    const imageRatio = imageWidth / imageHeight;

    let shrankImageWidth = imageWidth;
    let shrankImageHeight = imageHeight;
    if (imageWidth > maxImgContainerWidth) {
      shrankImageWidth = maxImgContainerWidth;
      shrankImageHeight = maxImgContainerWidth / imageRatio;
    } else if (imageHeight > maxImgContainerHeight) {
      shrankImageHeight = maxImgContainerHeight;
      shrankImageWidth = maxImgContainerHeight * imageRatio;
    }

    this.setState({
      imgWidth: shrankImageWidth,
      imgHeight: shrankImageHeight,
      isLoaded: true
    });
  }
...
export default XTextPreviewBox;
