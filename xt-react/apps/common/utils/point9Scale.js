...
function computedImageParamsInCanvas(images, targetWidth, targetHeight, paddings, outPaddings) {
  ...
  // 期望的宽加上左侧白边的宽
  const paddingsInContainer = getPaddingsInContainer(targetWidth,
    targetHeight,
    paddingRatios.top,
  ...
  const outPaddingsInContainer = getPaddingsInContainer(targetWidth,
    targetHeight,
  ...
  const containerWidth = Math.ceil(targetWidth + paddingsInContainer.left + paddingsInContainer.right);

  // 期望的高加上下宽白边的高.
  const containerHeight = Math.ceil(targetHeight + paddingsInContainer.top + paddingsInContainer.bottom);
...
export const combine = (images, targetWidth, targetHeight, paddings, outPaddings, cb) => {
  ...
  init(images, (allImages) => {
    ...
    const imagesOptionsInCanvas = computedImageParamsInCanvas(allImages, targetWidth, targetHeight, paddings, oPaddings);
