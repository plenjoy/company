...
const prepareOptionGroup = (specObj) => {
  ...
  if (optionArray && optionArray.forEach) {
...
const prepareKeyPatternData = (dataArray) => {
  ...
  let fixedDataArray = dataArray;
  if (typeof dataArray === 'undefined') {
    return outArray;
  } else if (!dataArray.length) {
    fixedDataArray = [dataArray];
  }

  fixedDataArray.forEach((optionMap) => {
      ...
      keyPatternArray.forEach((key, index) => {
        ...
        if (keyByKeyPattern.startsWith('[') && keyByKeyPattern.endsWith(']')) {
          keyByKeyPattern = keyByKeyPattern
            .substring(1, keyByKeyPattern.length - 1)
            .split(',');
        } else if (
          keyByKeyPattern.indexOf(',') !== -1 &&
