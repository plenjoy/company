...
export const getSettingLabel = (setting, selectMap) => {
  return {
    ...
    paperThicknessLabel: getItemLabel(setting.paperThickness, selectMap.paperThicknessMap)
  };
};
...
export const downLoadSpec = (that) => {
  ...
  const { boundProjectActions, boundConfirmModalActions, cancelSetting } = actions;
  let downLoadSpecUrl = '';
  if (that.state.isInEdit && !isEqual(that.state.oldSetting, that.props.data.setting)) {
    ...
    const baseUrl = that.props.data.baseUrl;
    const coverName = that.getCoverName();
    downLoadSpecUrl = template(DOWNLOAD_BOOKSPEC_URL)({
      baseUrl,
      bookName: that.props.data.title,
      cover: coverName,
      size: that.props.data.setting.size
    });
...
export const routerClickHandle = (that, event) => {
  ...
  if (target.parentNode.className ==='item') {
    const { actions, t } = that.props;
    const { boundProjectActions, boundConfirmModalActions, cancelSetting, saveSetting } = actions;
