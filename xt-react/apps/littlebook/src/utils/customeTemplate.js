...
export const getTemplateIdOnCreate = (userId, imageObj) => {
  const isSwitched = Math.abs((parseInt(imageObj.orientation || 0) / 90) % 2) === 1;
  const w = parseInt(imageObj.width);
  const h = parseInt(imageObj.height);

  const imageWidth = isSwitched ? h : w;
  const imageHeight = isSwitched ? w : h;
