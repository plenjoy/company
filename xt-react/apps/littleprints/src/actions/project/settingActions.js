...
import { productTypes, elementTypes, pageTypes, limitedPageNum } from '../../constants/strings';
...
function transformPhotoElement(element, imageArray, pageWidth, pageHeight) {
  ...
  if (imageDetail) {
    ...
    const imageWidth = Math.abs(imageRotate) === 90
      ? imageDetail.get('height')
      : imageDetail.get('width');
    const imageHeight = Math.abs(imageRotate) === 90
      ? imageDetail.get('width')
      : imageDetail.get('height');
    const newFitCrop = getFitCrop(imageWidth,
      imageHeight,
      element.get('cropLUX'),
      element.get('cropLUY'),
...
export function transformPageArray(existsPageArray, newPageArray, newParameterMap, imageArray) {
  ...
  let resultPageArray = existsPageArray.map((page) => {
    const isLandscapeShape = page.get('width') > page.get('height');
    return page.merge({
      width: isLandscapeShape ? firstNewPage.get('height') : firstNewPage.get('width'),
      height: isLandscapeShape ? firstNewPage.get('width') : firstNewPage.get('height'),
...
export function transformCover(newCover, newParameterMap, oldCover, imageArray) {
  ...
  resultContainers.forEach((page, index) => {
    resultContainers = resultContainers.setIn(
      [String(index), 'elements'], transformElementArray(page, imageArray)
    );
  });
...
export function computeNewData(stateData, newSetting = null, isOldProject=false) {
  ...
  const pageArray = generatePageArray(productType, convertedParameterMap);
