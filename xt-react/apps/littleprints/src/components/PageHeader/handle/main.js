...
export const onOrder = (that) => {
  ...
  boundTrackerActions.addTracker(`TapOrder`);
  ...
  if (trackerBlankNum > 0) {
    boundConfirmModalActions.showConfirm({
      confirmMessage: (
        <div className="text-left">
          The prints you are ordering can contain up to {limitedPageNum} pieces.
          Currently it contains only {limitedPageNum-trackerBlankNum} piece(s).
          You have {trackerBlankNum} blank piece(s). Click continue for prints to be printed with blank pieces.
        </div>
      ),
      onOkClick: () => {
        // order 时提示有空白页 仍点击 continue 的埋点;
        boundTrackerActions.addTracker(`ClickOrderAndContinue,${trackerBlankNum}`);
        if (setting.get('size') === '7X5' && setting.get('product') === productTypes.LPS) {
    ...
    boundTrackerActions.addTracker(`ClickOrderAndContinue,${trackerBlankNum}`);
    if (setting.get('size') == '7X5' && setting.get('product') == 'LPS') {
      boundUpgradeModalActions.showUpgrade(onSaveProjectFun);
      that.isOrdering = false;
    } else {
      onSaveProject(() => {}, () => {
...
export const onPreview = (that) => {
  ...
  const { boundPreviewModalActions,boundTrackerActions } = actions;
   boundTrackerActions.addTracker('TapPreview')
