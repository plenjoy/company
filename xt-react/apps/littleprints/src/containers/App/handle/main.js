  ...
  if (projectId !== newProjectId) {
    ...
    boundEnvActions
      ...
      .then(() => {
        if (projectId === -1) {
          boundProjectActions.projectLoadCompleted();
          boundTrackerActions.addTracker(`ProjectCreateComplete`);
...
export function saveProjectWithOutScreenShot(
  that,
  onSaveSuccessed,
  onSaveFailed,
  isFirstTime = false,
      ...
      if (isRequestSuccess) {
        const promise = new Promise((resolve, reject) => {
          ...
          if (guid) {
            ...
            const getScreenShotSelecter =
              '#app > div > div.book-cover-screenshot  canvas';
            getScreenShot(getScreenShotSelecter, isFirstTime).then(url => {
              if (!url && !isFirstTime) {
                boundTrackerActions.addTracker('GetScreenShotFailed');
              }
    ...
    .then(res => {
      ...
      if (isRequestSuccess) {
        ...
        if (guid) {
          ...
          getScreenShot('#app > div > div.book-cover-screenshot  canvas').then(
            url => {
              boundProjectActions.uploadCoverImage(url).then(() => {
                if (onCloneSuccessed) {
                  onCloneSuccessed();
