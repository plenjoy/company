...
import { getTransferDataByNode, getTransferData } from '../../../../../common/utils/drag';
import Element from '../../../utils/entries/element';
import capabilities from '../../../reducers/system/capabilities/index';
...
function getElementFromPoint(container, elementArray, x, y, isSpinePage) {
  // let newElementArray = elementArray.filter(ele => ele.get('type') !== elementTypes.background);
  let newElementArray = setSpineMaxDep(elementArray);
...
export const addEventsToLayerOfElements = (that) => {
  ...
  const isShowCameo =
    variables && isCover && get(settings, 'spec.product') !== productTypes.PS
  ...
  if (that.stage) {
    ...
    container.addEventListener('mouseup', (ev) => {
      ...
      if (__app.isExchangeImage) {
        ...
        if (that.elementControlsNode) {
          const { isDragging, isRotating } = that.elementControlsNode;
    ...
    container.addEventListener('mousedown', (ev) => {
      ...
      if (element && element.get('type') !== elementTypes.background) {
        ev.stopPropagation();
        ...
        if (that.clicksObj.num === 1) {
          window.setTimeout(() => {
            if (that.clicksObj.num === 1) {
              ...
              // 解决background双击不响应问题
              // double click
              if (elementId === that.clicksObj.clickElementId || element.get('type') === elementTypes.background) {
                switch (element.get('type')) {
                  ...
                  case elementTypes.background: {
                    const hasImage = Boolean(element.get('backgroundId'));
                    if (hasImage) {
                      onEditImage(that, element, ev);
                    }
                    break;
    ...
    const handleDragOver = (ev) => {
      ...
      const { elementArray, containerRect } = that.state;
      // 过滤掉backgroundelement
      const newElementArray = elementArray.filter(
      ...
      if (
        ...
        if (element) {
          if (
            ...
            element.get('type') === elementTypes.sticker ||
    ...
    const handleDrop = (ev) => {
      ...
      if (!dragData || (!page.get('enabled') && !element)) {
        that.setState({
          isDragOver: false
        });
        return false;
      ...
      if (__app.isExchangeImage) {
        if (__app.dragType !== dragTypes.template) {
          ...
          if (element && element.get('type') === elementTypes.photo) {
            ...
            if (dragData.elementId && dragData.elementId !== elementId) {
              ...
              boundTrackerActions.addTracker('SwapImage');
            }
            // 重置状态
            that.setState({
              isDragOver: false,
        ...
        delete __app.dragType;
      } else if (__app.dragType === dragTypes.theme) {
        // 如果当前拖拽的theme
        pageHandler.onPageDroped(that, ev);
        delete __app.dragType;
      } else if (__app.dragType === dragTypes.sticker) {
        ...
        pageHandler.onPageDroped(that, ev);
        delete __app.dragType;
      } else if (__app.dragType === dragTypes.background) {
        const enabled = page.get('enabled');
        // 如果页面为disable, 就不要添加任何事件.
        if (!enabled) {
          return;
