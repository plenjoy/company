...
import { BACKGROUND_SRC } from '../../../contants/apiUrl';
...
export const onEditImage = (that, element, e) => {
  ...
  const { images, elements, ratio, backgroundArray, stickerArray, env } = data;
  ...
  let imageDetail;
  if (backgroundId) {
    imageDetail = backgroundArray.find((b) => {
      return b.get('code') === backgroundId;
    });
    // 此处为了定义的接口规范统一，将background原始图片的地址传入encimageid
    encImgId = element.getIn(['computed', 'sourceUrl']);
  } else {
    imageDetail = images.get(encImgId);
  }
...
export const onFilter = (that, element, e) => {
  ...
  const { boundPropertyModalActions, boundTrackerActions } = actions;
...
export const onRemoveImage = (that, element, e) => {
  ...
  if (element.get('type') === elementTypes.photo) {
    ...
    boundTrackerActions.addTracker('ClickRemoveFrame');
...
export const onBringToFront = (that, element, e) => {
  ...
  boundTrackerActions.addTracker('ClickBringToFront');

  const newElements = elements.filter(ele => ele.get('type') !== elementTypes.background && ele.get('id') !== element.get('id'));
  const nearest = newElements.maxBy((item) => {
    return item.get('dep');
  });
  if (
...
export const onSendToback = (that, element, e) => {
  ...
  const { boundProjectActions, boundTrackerActions } = actions;
  const { elements } = data;
  boundTrackerActions.addTracker('ClickSendToback');

  const newElements = elements.filter(ele => ele.get('type') !== elementTypes.background && ele.get('id') !== element.get('id'));
  const nearest = newElements.minBy((item) => {
    return item.get('dep');
  });
  if (
    element.get('type') === elementTypes.cameo ||
...
export const onBringForward = (that, element, e) => {
  ...
  boundTrackerActions.addTracker('ClickBringForward');
...
export const onSendBackward = (that, element, e) => {
  ...
  const { elements } = data;
  const { boundProjectActions, boundTrackerActions } = actions;
  boundTrackerActions.addTracker('ClickSendBackward');
...
export const onEditText = (that, element, e) => {
  ...
  const { boundTextEditModalActions, boundPaintedTextModalActions, boundTrackerActions } = actions;

  boundTrackerActions.addTracker('ClickEditText');
...
export function onFitFrameToImage(that, element, e) {
  ...
  boundTrackerActions.addTracker('ClickFitFrameToImage');
...
export function onAlignLeft(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickAlignLeft');
...
export function onAlignCenter(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickAlignCenter');
...
export function onAlignRight(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickAlignRight');
...
export function onAlignTop(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickAlignTop');
...
export function onAlignMiddle(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickAlignMiddle');
...
export function onAlignBottom(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickAlignBottom');
...
export function onSpaceHorizontal(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickSpaceHorizontal');
...
export function onSpaceVertical(that, selectedElementArray) {
  ...
  boundTrackerActions.addTracker('ClickSpaceVertical');
...
export const onMatchWidestWidth = (that, selectedElementArray) => {
  ...
  boundTrackerActions.addTracker('ClickMatchWidestWidth');
...
export const onMatchNarrowestWidth = (that, selectedElementArray) => {
  ...
  boundTrackerActions.addTracker('ClickMatchNarrowestWidth');
...
export const onMatchTallestHeight = (that, selectedElementArray) => {
  const { actions } = that.props;
  const { boundProjectActions, boundTrackerActions } = actions;
  boundTrackerActions.addTracker('ClickMatchTallestHeight');
...
export const onMatchShortestHeight = (that, selectedElementArray) => {
  ...
  boundTrackerActions.addTracker('ClickMatchShortestHeight');
...
export const onMatchFirstSelectWidthAndHeight = (
  ...
  boundTrackerActions.addTracker('ClickMatchFirstSelectWidthAndHeight');
