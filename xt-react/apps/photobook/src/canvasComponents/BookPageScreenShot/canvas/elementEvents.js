...
import { getTransferData, setTransferDataByNode, getTransferDataByNode } from '../../../../../common/utils/drag';
...
export const onMouseEnter = (that, element, e) => {
  ...
  if (!element.get('text') && !isSelectMousePointElement &&
    textTypeList.indexOf(element.get('type')) !== -1) {
    const elementNode = findKonvaObjectById(
      that.unselectElements, element.get('id')
    );
...
export const onDblClick = (that, element, ev) => {
