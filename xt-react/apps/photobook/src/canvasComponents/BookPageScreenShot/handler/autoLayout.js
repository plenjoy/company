...
const doApplyTemplate = (that, template, nextProps) => {
  ...
  const { elements, images, page, summary, ratio, settings } = data;
  ...
  const textOrPhotoElements = elements.filterNot(ele => ele.get('type') === elementTypes.sticker);
  const emptyTextElements = elements.filter(ele => ele.get('type') === elementTypes.text && !ele.get('text'));
  const stickersElements = elements.filter(ele => ele.get('type') === elementTypes.sticker);
  ...
  if (textInTemplate.length) {
    ...
    if (newElements && newElements.size) {
      if (emptyTextElementIds && emptyTextElementIds.length) {
        return boundProjectActions.deleteElements(emptyTextElementIds).then(() => {
...
export const doAutoLayout = (that, props) => {
  ...
  if (isEnableAutoLayout && elements.size) {
    ...
    const templateOverView = autoLayoutByElements(elements, allTemplatesList, boundTrackerActions, {size: projectSize, isCover});
