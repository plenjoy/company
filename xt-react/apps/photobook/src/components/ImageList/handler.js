...
export const convertGroupImageData = (images, uploadBaseUrl,userInfo) => {
  const newImages = {};
  for (const key in images) {
    if (images[key] && images[key].map) {
      newImages[key] = images[key].map((v) => {
        const value = v;
        value.src = v.encImgId ? combine(uploadBaseUrl, IMAGE_SRC, {
          qaulityLevel: 0,
          puid: v.encImgId,
...
export const onDragStarted = (that, id, event) => {
  ...
  const images = that.props.uploadedImages;
  let uploadedImages = [];
  for (const key in images) {
    uploadedImages = uploadedImages.concat(images[key]);
  }
...
export const addGroupElement = (that, imageObj) => {
  const { addGroupElement } = that.props;

  if (imageObj.encImgId && addGroupElement) {
    addGroupElement(imageObj);
    onOutImageItem(that);
  }
};

export const addGroupElements = (that, key) => {
  const { addGroupElements } = that.props;

  if (addGroupElements) {
    addGroupElements(key);
    onOutImageItem(that);
  }
};
...
const reCalsScale = (width, height) => {
  ...
  if (width >= height) {
    ...
    return {
      ...
      height: Math.round(rWidth / ratio)
    };
  }
  const rHeight = MAX_HEIGHT;

  return {
    width: Math.round(rHeight * ratio),
