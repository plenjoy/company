...
class OptionalButtonList extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    const newStyle = merge({}, style, {
      display: isShowPaintedText || isShowCameo ||  isShowSpineText || isShowTakeShot ? 'block' : 'none'
    });
    ...
    return (
        ...
        {/* 虽然这个也叫SPINE_TEXT 但是 实际内容却不一样 */}
      {
       ...
       {
       isShowTakeShot? (
         <XButton className={addCameoClassName} onClicked={actions.saveCurrentPageScreenshot}>{t('TAKE_SCREENSHOT')}</XButton>
        ) : null
      }
...
OptionalButtonList.defaultProps = {
  data: {
    isShowAddCameo: true,
...
export default translate('OptionalButtonList')(OptionalButtonList);
