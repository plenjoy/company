...
function generatePage(parameterMap, pageType, additionalOptions) {
  ...
  const { month, year, isPosterCalendar } = additionalOptions;

  const innerPageSize = getInnerPageSize(baseSize, pageBleed);

  const elements = [];
  if (pageType === pageTypes.bottom || isPosterCalendar) {
    elements.push({
      id: guid(),
...
function generateCover(
  ...
  const isPosterCalendar = productType === productTypes.PC;
  const additionalOptions = {
    isPosterCalendar,
    month: isPosterCalendar ? 1 : 0,
...
function generatePageArray(
  ...
  if (!parameterMap || productType === productTypes.PC) return [];
