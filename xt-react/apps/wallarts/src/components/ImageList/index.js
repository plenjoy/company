...
class ImageList extends Component {
        ...
        imageInRow.forEach((imageObj, i) => {
          ...
          subHtml.push(
            <div key={imageObj.guid} className="lazy-item" style={isSelectedStyle}>
              <XDrag onDragStarted={this.onDragStarted.bind(this, imageObj.guid)}>
  ...
  render() {
    const { uploadedImages, baseUrls, className } = this.props;
    const { magnifierParams } = this.state;
    const twoDimenImagesArr = convertImageData(uploadedImages, baseUrls.get('uploadBaseUrl'));
    const imageListClass = classNames('image-list', className);
    return (
      <div>
        <div className={imageListClass} ref="imageList" onMouseUp={this.onImageListDown}>
          {
