...
import { leastUploadImagesCount, enumOrientation } from '../../../../common/utils/strings';
...
const saveFileToStore = (that, file, res) => {
  ...
  const orientation = getDegree(get(res, 'resultData.img.exifOrientation'));
  ...
  if (autoAddPhotoToCanvas && autoAddPhotoToCanvas.status) {
    const { elementId, projectOrientation, shouldAutoChangeOrientation } = autoAddPhotoToCanvas;

    const isSwitched = Math.abs((fileInfo.orientation / 90) % 2) === 1;

    const w = isSwitched
      ? parseInt(fileInfo.height)
      : parseInt(fileInfo.width);
    const h = isSwitched
      ? parseInt(fileInfo.width)
      : parseInt(fileInfo.height);

    const imageOrientation = w >= h ? 'Landscape' : 'Portrait';
    const isSquareImage = fileInfo.width === fileInfo.height;
    // 获取图片的裁剪参数.
    const element = {
      id: elementId,
      encImgId: get(res, 'resultData.img.encImgId'),
    ...
    if (shouldAutoChangeOrientation &&
  ...
  if (newImages) {
    ...
    newImages.forEach((file, index) => {
      ...
      const reducer = instance.push({
        ...
        error: err => {
          const { countOfFailed, countOfCompleted,checkIsAllCompleted } = that.state.instance;
