...
module.exports = {
	template : '<div id="tmpl" v-bind:style="{zIndex:windowZindex,width:width}">' +
			   		'<div class="tmpl_header">' +
			   			'<label v-on:click="toggleShow()" v-bind:class="{\'expand\':sharedStore.isChangeTmplExpanded}">{{ selectText }}</label>' +
			   			'<a href="javascript:void(0);" v-show="showChangeTmpl && isTmplShow" v-on:click="showChange()" style="margin-left: 30px;" v-bind:class="{\'current\':sharedStore.isChangeTmplShow}">{{ privatedStore.changeButtText }}</a>' +
			   			'<label style="float: right;margin-right: 30px;background: none;"><input type="checkbox" v-model="privatedStore.isAutoLayout"> {{ privatedStore.autoText }}</label>' +
