...
module.exports = {
        ...
        if (projectJson) {
            ...
            for (var k = 0; k < projectJson.pages.length; k++) {
                ...
                if(Store.projectSettings[Store.currentSelectProjectIndex].product) {
                    ...
                    currentCanvas.canvasBordeThickness = JsonProjectManage.getCanvasBorder(k);
    ...
    fixResizePhotoElement: function(pageIdx) {
        ...
        var currentCanvas = Store.pages[useIndex].canvas;
        var realBleedings = JsonProjectManage.getBleedSize(useIndex);
        var frameBaseSize = JsonProjectManage.getBaseSize(useIndex);
        var canvasBordeThickness = JsonProjectManage.getCanvasBorder(useIndex);
        var mirrorSize = { top: 0, right: 0, bottom: 0, left: 0 };
        var photoLayer = JsonProjectManage.getPhotoLayer(useIndex);
    ...
    getForeground : function(frameBaseSize, expendSize,photoLayer,projectIdx){
        ...
        changeBorderToMirror: function(isWrap,isOptionChange) {
            ...
            if(!Store.isMirrorBorder && !Store.isCanvas){
                return;
            }
            var currentCanvas = Store.pages[Store.selectedPageIdx].canvas;
            var photoLayer = JsonProjectManage.getPhotoLayer(Store.selectedPageIdx);
            var borderThickness = JsonProjectManage.getCanvasBorder(Store.selectedPageIdx);
            var realBleedings = JsonProjectManage.getBleedSize(Store.selectedPageIdx);
