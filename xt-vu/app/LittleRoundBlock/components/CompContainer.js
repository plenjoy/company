                ...
                '<div class="flipper" v-bind:style="flipperStyle" style="margin: 0 auto;">'+
                    '<div v-show="!main || isFrontPageShow" id="box-canvasbg{{idx}}{{main ? \'Main\' : \'\'}}" v-bind:style="frontPageStyle" style="position: absolute;overflow: hidden;z-index:2;">' +
                      ...
                      '<div style="position: relative;width: 100%; height: 100%; overflow: hidden;">' +
                        '<div class="bed-operation" id="container{{idx}}{{main ? \'Main\' : \'\'}}" v-bind:style="{ top: privateStore.canvasTop + \'px\', left: privateStore.canvasLeft + \'px\' }" style="position: relative; overflow: hidden;background: #f0f0f0;">' +
                        ...
                        '</div>' +
                      '</div>' +
                      '<bg-layer v-show="!sharedStore.isCanvas" v-bind:idx="idx" v-bind:ratio="privateStore.ratio" v-bind:main="main" v-bind:width="Math.floor(privateStore.operationWidth)" v-bind:height="Math.floor(privateStore.operationHeight)" v-bind:original-bg-width="privateStore.originalBgWidth"></bg-layer>' +
  ...
  methods: {
    ...
    initWindow: function() {
      ...
      if(boxLimit.width > 0 && boxLimit.height > 0) {
        ...
        _this.privateStore.originalBgWidth = currentCanvas.oriBgWidth;
      }
      else {
    ...
    handleDeleteProject: function(){
      ...
      if(hasImage) {
          this.sharedStore.vm.$dispatch("dispatchShowPopup", { type : 'deleteProject', status : 0, pageIdx: this.idx,message:"This operation will delete this block, would you like to continue?" });
      } else {
