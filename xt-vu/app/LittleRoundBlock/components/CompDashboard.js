...
module.exports = {
  ...
  methods: {
    ...
    fillImageIntoBlankPage: function(imgArray){
      ...
      imgArray.forEach(function(imageDetail) {
        ...
        // 从baseProject中同步属性
        var settingKeys = Object.keys(Store.baseProject);

        settingKeys.forEach(function(settingKey) {
          project[settingKey] = Store.baseProject[settingKey];
        });
        ...
        var canvasBordeThickness = JsonProjectManage.getCanvasBorder(length - 1);
        var oriX = foreground.x;
        var oriY = foreground.y;
  ...
  events: {
    ...
    notifyAddNewUploadedImgIntoPages: function() {
      if(this.sharedStore.newUploadedImg.length !== 0) {
