module.exports = {
    ...
    template: '<div style="position:absolute; box-sizing: border-box;" @click="changePageIdx" v-bind:style="usedStyle">' +
                    ...
                    '</div>' +
                    '<warntip-element v-bind:id="id" v-bind:pagedd="pageIdx" v-bind:main="extraName" v-if="!sharedStore.isPreview" v-bind:ratio="ratio" v-bind:width="elementData.width*ratio" v-bind:height="elementData.height*ratio" v-bind:x="warnTipX" v-bind:y="elementData.y*ratio"></warntip-element>'+
    ...
    computed: {
        warnTipX: function() {
            var shape = Store.projectSettings[this.pageIdx].shape;
            ...
            if(shape === 'Round') {
                ...
                var offset = Store.warnTipMargin * this.ratio;
                
                var offsetX = Math.abs(currentCanvas.photoLayer.x) * this.ratio;
                if(this.elementData.x * this.ratio < offsetX){
                    offset += offsetX - this.elementData.x * this.ratio + 9;
                }
