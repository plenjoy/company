...
module.exports = {
	template: '<div v-show="sharedStore.watches.isProjectLoaded" v-bind:style="topStyle" class="bed-actionpanel-bottom">' +
                '<div style="height: 34px;text-align: center;">' +
                  '<div class="action-item" v-on:click="changePage(currentPage !== \'Front\')" v-bind:style="{ marginTop:marginTop }" style="cursor: pointer;background:white;border:1px solid #d6d6d6; width: 140px;height: 22px;line-height: 22px;border-radius: 4px;overflow:hidden;">' +
                    '<span style="width:20px;margin-right: 6px;margin-top:6px;font-size: 0px;">'+
                        '<img src="./assets/img/icon/fresh.svg" height="12" width="13" alt="Change Page" title="Change Page" style="position: relative; transform: translateY(1px);" />' +
                    '</span>' +
