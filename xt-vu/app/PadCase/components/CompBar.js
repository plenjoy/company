...
module.exports = {
  template: '<div class="box-bar"  v-bind:style="{left: boxLeft + \'px\', top: boxTop + \'px\', zIndex: windowZindex }" style="width:122px;">' +
              ...
              // '</span>' +
               '<span class="button" v-show="type === \'image\' && hasImage" v-on:click="handleEditImage()" title="Click to crop image" style="isplay: inline-block;width:100%; height: 26px;line-height: 26px;padding-left: 8px;text-align:left;font-size:12px;border-radius: 5px 5px 0 0;">' +
                'Crop Image'+
  ...
  computed: {
    ...
    boxLeft: function() {
      ...
      var currentCanvas = Store.pages[Store.selectedPageIdx].canvas,
          ...
          bleed = ProjectManage.getPhonecaseBleed(),
          edge = ProjectManage.getPhonecaseEdge(),
          side = ProjectManage.getPhonecaseSide(),
          baseSize = ProjectManage.getPhonecaseBaseSize();
      if(((parentEl.x+bleed.left+edge.left+side.left)*currentCanvas.ratio)>this.sharedStore.barPosition.x){
        return (parentEl.x+bleed.left+edge.left+side.left)*currentCanvas.ratio
      }else if(((parentEl.x+bleed.left+edge.left+side.left+baseSize.width)*currentCanvas.ratio-130)<this.sharedStore.barPosition.x){
        return (parentEl.x+bleed.left+edge.left+side.left+baseSize.width)*currentCanvas.ratio - 130;
      }else{
    ...
    boxTop: function() {
      ...
      var currentCanvas = Store.pages[Store.selectedPageIdx].canvas,
          ...
          side = ProjectManage.getPhonecaseSide(),
          baseSize = ProjectManage.getPhonecaseBaseSize();
      if(((parentEl.y+bleed.top+edge.top+side.top)*currentCanvas.ratio)>this.sharedStore.barPosition.y){
        return (parentEl.y+bleed.top+edge.top+side.top)*currentCanvas.ratio
      }else if(((parentEl.y+bleed.top+edge.top+side.top+baseSize.height)*currentCanvas.ratio-78)<this.sharedStore.barPosition.y){
        return (parentEl.y+bleed.top+edge.top+side.top+baseSize.height)*currentCanvas.ratio - 78;
      }else{
