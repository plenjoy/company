...
module.exports = {
  ...
  //   if((currentCanvas.params[idx].elType === 'background' || currentCanvas.params[idx].elType === 'image' || currentCanvas.params[idx].elType === 'logo') && imageDetail) {
	...
	initProject: function(sXml) {
		if(sXml) {
			for(var k = 0; k < $(sXml).find('product').length; k++) {
				// var prj = Store.projectSettings[k];
				// var specData = SpecController.analyseSpec({ size: prj.size, product: prj.product});

				Store.projects[k] = {};
				Store.projects[k].pages = [];
				for(var i = 0; i < $(sXml).find('product').eq(k).find('content').length; i++) {
					Store.projects[k].pages.push({
						type: $(sXml).find('product').eq(k).find('content').eq(i).attr('type') || '',
						name: '',
						canvas: {
							// isInited: false,
					...
					var spread = require("ProjectManage").getContent();
					...
					var paramsCount = $(sXml).find('product').eq(k).find('content').eq(i).find('element').length;
					for(var j = 0; j < paramsCount; j++) {
						var imgId = $(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('imageid') || '',
						...
						var ox = parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('x')) || 0,
								oy = parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('y')) || 0,
								ow = parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('width')) || 0,
								oh = parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('height')) || 0;
						...
						var elType = '';
						if($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('type') === 'PhotoElement') {
							elType = 'image';
						}
						else if($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('type') === 'TextElement') {
							elType = 'text';
						};
						...
						currentCanvas.params.push({
							...
							url: '',
							isRefresh: false,
							text: decodeURIComponent($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).text()) || '',
							// x: currentCanvas.oriWidth * (parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('px')) || 0),
							x: ox,
							// y: currentCanvas.oriHeight * (parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('py')) || 0),
							y: oy,
							// width: currentCanvas.oriWidth * (parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('pw')) || 0),
							width: ow,
							// height: currentCanvas.oriHeight * (parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('ph')) || 0),
							height: oh,
							rotate: parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('rot')),
							dep: parseInt($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('dep')),
							imageId: imgId,
							imageGuid: imageDetail.guid || '',
							imageWidth: imageDetail.width || '',
							imageHeight: imageDetail.height || '',
							imageRotate: parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('imgRot')) || 0,
							// imageFlip: ,
							cropPX: parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('cropLUX')) || 0,
							cropPY: parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('cropLUY')) || 0,
							cropPW: (parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('cropRLX')) - parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('cropLUX'))) || 1,
							cropPH: (parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('cropRLY')) - parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('cropLUY'))) || 1,
							fontFamily: decodeURIComponent($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('fontFamily')) || '',
							fontSize: currentCanvas.oriHeight * parseFloat($(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('fontSize') || 0),
							fontWeight: $(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('fontWeight') || '',
							textAlign: $(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('textAlign') || '',
							fontColor: $(sXml).find('product').eq(k).find('content').eq(i).find('element').eq(j).attr('color') || ''
						});
					};
				};
			};
