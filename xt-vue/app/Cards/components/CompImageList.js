...
module.exports = {
	...
	template:   '<div style="text-align: center;" v-on:click="blurFocus">' +
					...
					'<div class="sort-hide" v-show="isImageListShow > 0" style="width:260px;height:20px;line-height:20px;margin:8px auto 0">'+
						'<div class="sort-image" style="float:left;" >'+
							'<span class="sort-label" @click="setSortDropdown(\'toggle\')" v-el:sortBy>Sort By</span>' +
							'<div class="sort-dropdown" v-show="privateStore.isDropdownOpen">' +
								'<div class="sort-dropdown-item" @click="sortImage(\'uploadTimeN2O\')" :class="{active: privateStore.selected === \'uploadTimeN2O\'}">Upload Time (newest to oldest)</div>'+
								'<div class="sort-dropdown-item" @click="sortImage(\'uploadTimeO2N\')" :class="{active: privateStore.selected === \'uploadTimeO2N\'}">Upload Time (oldest to newest)</div>'+
								'<div class="sort-dropdown-item" @click="sortImage(\'createTimeN2O\')" :class="{active: privateStore.selected === \'createTimeN2O\'}">Date Taken (newest to oldest)</div>'+
								'<div class="sort-dropdown-item" @click="sortImage(\'createTimeO2N\')" :class="{active: privateStore.selected === \'createTimeO2N\'}">Date Taken (oldest to newest)</div>'+
								'<div class="sort-dropdown-item" @click="sortImage(\'nameA2Z\')" :class="{active: privateStore.selected === \'nameA2Z\'}">Title (A - Z)</div>'+
								'<div class="sort-dropdown-item" @click="sortImage(\'nameZ2A\')" :class="{active: privateStore.selected === \'nameZ2A\'}">Title (Z - A)</div>'+
							'</div>' +
						'</div>'+
						'<div class="hide-image" style="float:right;">'+
					...
					'<div id="list-image" v-bind:style="{overflow: newImageList.length ? \'auto\': \'\' }">' +
							...
							'<div v-show="newImageList.length" class="item-image" v-for="item in newImageList" v-on:mouseover="handleShowDeleteIcon($index)" v-on:mouseout="handleHideDeleteIcon($index)">'+
								'<div class="wrap-image">'+
									'<div class="loaded-image">'+
										...
										'<div class="box-preview-image" v-bind:style="{opacity: item.usedCount > 0? 1: 0 }">' +
											'<span>{{ item.usedCount }}</span>' +
										'</div>'+
										'<div>'+
											'<img class="icon-delete" id="icon-delete-{{ $index }}" src="./assets/img/delete.svg" v-on:click="handleDeleteImage($index)" v-show="item.usedCount <= 0" alt="delete" title="delete" style=" opacity: 0; cursor: pointer;" />'+
											'<img class="icon-delete" src="./assets/img/delete.svg" v-else alt="delete" style=" opacity: 0; cursor: pointer;" />' +
										'</div>'+
									'</div>'+
								'</div>'+
							'</div>'+
						...
						'<div v-show="!sharedStore.imageList.length" class="item-empty" v-on:click="triggerImageUpload()">'+
	...
	computed: {
		newImageList: function() {
			...
			var item =this.privateStore.selected;
	...
	methods: {
		...
		// },
		setSortDropdown: function(status) {
			switch(status) {
				case 'toggle':
					this.privateStore.isDropdownOpen = !this.privateStore.isDropdownOpen;
					break;
				default:
					this.privateStore.isDropdownOpen = status;
					break;
			}
		},
		sortImage: function(dir) {
			this.privateStore.selected = dir;
			require('trackerService')({ev: require('trackerConfig')['ChangeSort'], filter: dir});
		},
		...
		handleHideUsedToggle: function() {
			var hideUsedStatus = this.privateStore.isHideUsed ? 'Off' : 'On';
			require('trackerService')({ev: require('trackerConfig')['ChangeHideUsed'], isHideUsed: hideUsedStatus});
		},
		...
		closeSortBy: function(event) {
			if(event.target !== this.$els.sortby) {
	...
	events: {
		...
		notifyImageList: function() {
			...
			UtilWindow.initImageListSizeWithCards(this.privateStore.imageListParams);
	...
	created: function() {
		document.addEventListener('click', this.closeSortBy.bind(this));
	},
	beforeDestroy: function() {
		document.removeEventListener('click', this.closeSortBy.bind(this));
	}
};
