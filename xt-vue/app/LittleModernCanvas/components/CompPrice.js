...
module.exports = {
	template:
				'<div v-if="!sharedStore.isPortal && !sharedStore.isPreview && oriPrice && !sharedStore.isRemark" v-bind:style="{zIndex:windowZindex}" style="position:absolute;right:40px;top:12px;">'+
	      			...
	      			'</div>'+
	      			'<div v-show="sharedStore.isTotalPriceShow" v-bind:style="{zIndex:windowZindex}" style="position:absolute;right:-20px;top:26px;width:220px;box-sizing:border-box;border:1px solid #d6d6d6;box-shadow:0 3px 6px rgba(0,0,0,.16);background:#fff;">'+
						...
						'<div v-for="item in allPriceItems" class="price-item-container" v-show="item.quantity">'+
							...
							'<span class="price-item font-light" style="float:left; width: 69px; box-sizing: border-box; white-space: nowrap;">{{getSizeTitleById(item.size)}}</span>'+
	...
	computed : {
		allPriceItems: function() {
			...
			// SpecManage.getOptions('shape').forEach(function(shape) {
				var paramArray = [
					{ key: 'product', value: 'littleModernCanvas'}
				];
		...
		getAllSizePrice: function() {
			if(Store.watches.isSpecLoaded && Store.watches.isProjectLoaded) {
				...
				_this.allSize.forEach(function(size) {
					...
					options = options.join(',');
					require("ProjectService").getLMCPrice(product, size, null, options, userId);
				});
			}
		},
