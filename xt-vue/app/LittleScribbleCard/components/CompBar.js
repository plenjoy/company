...
module.exports = {
  template: '<div class="box-bar"  v-bind:style="{left: boxLeft + \'px\', top: boxTop + \'px\', zIndex: windowZindex }" style="width:122px;">' +
              ...
              '</span>' +
              '<span class="button" v-show="type === \'image\' && hasImage" v-on:click="handleReplace(90)" title="Replace image" style="display: inline-block;width:100%; height: 26px;line-height: 26px;padding-left: 8px;text-align:left;font-size:12px;border-top: 1px solid #7b7b7b;">' +
                'Replace Image' +
                // '<img draggable="false" src="../../static/img/bar-rotate-right.png" alt="RR" style="width: 20px; height: 20px; margin-top: 5px;" />' +
              '</span>' +
  ...
  computed: {
    ...
    boxTop: function() {
      ...
      } else if(currentCanvas.oriHeight * ratio - 107 < this.sharedStore.barPosition.y){
        return this.sharedStore.barPosition.y - 107;

      }else if(-currentCanvas.oriY * ratio + currentCanvas.oriBgHeight * ratio - 107 < this.sharedStore.barPosition.y){
        return this.sharedStore.barPosition.y - 107;
  ...
  methods: {
    ...
    handleReplace: function() {
      this.sharedStore.watchData.replacePageId = Store.selectedPageIdx;
