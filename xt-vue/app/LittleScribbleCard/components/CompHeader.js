...
module.exports = {
    ...
    template: '<div class="bed-header" v-on:click="blurFocus">' +
                ...
                '<div style="float: right; height: 38px;margin-top: 0px;">' +
                  ...
                  '<span class="menu-item" v-show="!sharedStore.isRemark" v-on:click="handlePreview()" style="height: 38px;line-height: 38px;margin-right: 40px;">Preview</span>' +
        ...
        handleOrder: function() {
            ...
            if(Store.checkFailed){
                ...
                if(require('UtilProject').getIsShowProjectInfoView()){
                    ...
                    if(checkPageInfo.emptyPageNum){
                        var warnMessage = 'Little Scribble Card style you are ordering can contain up to ' +
                                            this.sharedStore.LSCPageNum + ' photo(s). Currently it contains only '+
                                            (this.sharedStore.LSCPageNum - checkPageInfo.emptyPageNum) +
                                            ' photo(s). You have '+
                                            checkPageInfo.emptyPageNum + (checkPageInfo.emptyPageNum > 1 ? ' blank cards.' : ' blank card.');
                        this.$dispatch("dispatchShowPopup", { type : 'alertMessage', message : warnMessage,height: 250});
                    } else {
                        require('ProjectController').saveOldProject(_this,function(){
            ...
            require('trackerService')({ev: require('trackerConfig').ClickOrder,emptyPageNum:checkPageInfo.emptyPageNum});
        },
