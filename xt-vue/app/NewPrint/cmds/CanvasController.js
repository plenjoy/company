...
module.exports = {
    createElement: function(idx,pageIdx, obj) {
        ...
        el.init(idx,pageIdx);
        el.$mount().$appendTo("#container"+pageIdx);
    ...
    initProject: function(projectJson) {
        ...
        if (projectJson) {
            // 初始化projects以及pages
            // projectJson的一个page生成一个项目project
            Store.projects[0] = {};
            Store.projects[0].pages = [];
            for (var j = 0; j < projectJson.pages.length; j++) {
                var currentPage = projectJson.pages[j];

                // 获取当前project下的基本参数
                var product = currentPage.spec.product;
                var size = currentPage.spec.size;
                var paper = currentPage.spec.paper;
                var quantity = currentPage.spec.quantity || 1;

                Store.projects[0].pages.push({
                    type: 'Print',
                    name: '',
                    idx : j,
                    guid: currentPage.id || require("UtilProject").guid(),
                    canvas: {
                        guid: currentPage.id || '',
                        oriWidth: 0,
                        oriHeight: 0,
                ...
                var rotated = (parseFloat(currentPage.width) / parseFloat(currentPage.height)<1) ? true : false;
                var currentCanvas = Store.projects[0].pages[j].canvas;

                currentCanvas.baseSize = JsonProjectManage.getPrintBaseSize({size: size, rotated: rotated});
                currentCanvas.bleedings = JsonProjectManage.getPrintBleed({size: size, rotated: rotated});
                currentCanvas.cornerRadius = JsonProjectManage.getPrintCornerRadius({product: product});
                currentCanvas.oriWidth = currentCanvas.baseSize.width + currentCanvas.bleedings.left + currentCanvas.bleedings.right;
                currentCanvas.oriHeight = currentCanvas.baseSize.height + currentCanvas.bleedings.top + currentCanvas.bleedings.bottom;
                currentCanvas.oriX = -currentCanvas.bleedings.left;
                currentCanvas.oriY = -currentCanvas.bleedings.top;
                currentCanvas.borderLength = currentPage.border ? currentPage.border.size : currentCanvas.borderLength;
                currentCanvas.borderColor = currentPage.border ? currentPage.border.color : currentCanvas.borderColor;

                var paramsCount = currentPage.elements.length;

                for (var i = 0; i < paramsCount; i++) {
                    var currentElement = currentPage.elements[i];
                    var imgId = currentPage.elements[i].imageid || '',
                    ...
                    var ox = parseFloat(currentElement.x) || 0,
                        oy = parseFloat(currentElement.y) || 0,
                        ow = parseFloat(currentElement.width) || 0,
                        oh = parseFloat(currentElement.height) || 0;
                    var px=parseFloat(currentElement.px) || 0;
                    var py=parseFloat(currentElement.py) || 0;
                    var pw=parseFloat(currentElement.pw) || 0;
                    var ph=parseFloat(currentElement.ph) || 0;
                    ox=px*currentCanvas.oriWidth;
                    oy=py*currentCanvas.oriHeight;
                    ow=pw*currentCanvas.oriWidth;
                    oh=ph*currentCanvas.oriHeight;
                    var elType = '';
                    ...
                    currentCanvas.params.push({
                        id: i,
                        guid: currentElement.id,
                        elType: elType,
                        // url: sourceImageUrl,
                        url: '',
                        isRefresh: false,
                        text: decodeURIComponent(currentElement.text) || '',
                        // x: currentCanvas.oriWidth * (parseFloat($(sXml).find('print').eq(k).find('elements').eq(i).find('element').eq(j).attr('px')) || 0),
                        x: ox,
                        // y: currentCanvas.oriHeight * (parseFloat($(sXml).find('print').eq(k).find('elements').eq(i).find('element').eq(j).attr('py')) || 0),
                        y: oy,
                        // width: currentCanvas.oriWidth * (parseFloat($(sXml).find('print').eq(k).find('elements').eq(i).find('element').eq(j).attr('pw')) || 0),
                        width: ow,
                        // height: currentCanvas.oriHeight * (parseFloat($(sXml).find('print').eq(k).find('elements').eq(i).find('element').eq(j).attr('ph')) || 0),
                        height: oh,
                        rotate: parseFloat(currentElement.rot),
                        dep: parseInt(currentElement.dep),
                        imageId: imgId,
                        imageGuid: imageDetail.guid || '',
                        imageWidth: parseInt(imageDetail.width) || '',
                        imageHeight: parseInt(imageDetail.height) || '',
                        imageRotate: parseFloat(currentElement.imgRot) || 0,
                        // imageFlip: ,
                        cropPX: parseFloat(currentElement.cropLUX) || 0,
                        cropPY: parseFloat(currentElement.cropLUY) || 0,
                        cropPW: (parseFloat(currentElement.cropRLX) - parseFloat(currentElement.cropLUX)) || 1,
                        cropPH: (parseFloat(currentElement.cropRLY) - parseFloat(currentElement.cropLUY)) || 1,
                        fontFamily: decodeURIComponent(currentElement.fontFamily) || '',
                        fontSize: currentCanvas.oriHeight * parseFloat(currentElement.fontSize || 0),
                ...
                // fix the depth value
                for(i = 0; i < currentCanvas.params.length; i++) {
                    currentCanvas.params[i].dep = i;
                };
            }
        };
    },
    ...
    loadProjectIntoPages: function(idx) {
        ...
        for (var i = 0; i < pickedProject.pages.length; i++) {
            Store.pages.push({
                ...
                canvas: {
                    guid:pickedProject.pages[i].canvas.guid ||'',
                    isInited: false,
                    ...
                    cornerRadius: pickedProject.pages[i].canvas.cornerRadius,
                    ...
                    bleedings: pickedProject.pages[i].canvas.bleedings, // bleeding sizes
                    ...
                    pageItems : [],
                    borderLength : pickedProject.pages[i].canvas.borderLength,
                    borderColor : pickedProject.pages[i].canvas.borderColor,
    ...
    syncProjectData: function(nProjectIdx) {
        ...
        for (var j = 0; j < Store.pages.length; j++) {
            ...
            // 	};
            // };
            // Store.projects[nProjectIdx].pages[j].idx = Store.pages[j].idx;
            Store.projects[nProjectIdx].pages[j].canvas.params = [];
            Store.projects[nProjectIdx].pages[j].canvas.params = currentCanvas.params.slice(0);
    ...
    freshPageData: function() {
        for (var k = 0; k < Store.pages.length; k++) {
            var currentCanvas = Store.pages[k].canvas;
            currentCanvas.baseSize = JsonProjectManage.getPrintBaseSize({size:Store.projectSettings[k].size,rotated:Store.projectSettings[k].rotated});
            currentCanvas.bleedings = JsonProjectManage.getPrintBleed({size:Store.projectSettings[k].size, rotated:Store.projectSettings[k].rotated});
            currentCanvas.baseSizeWithBleed = {
                width: currentCanvas.baseSize.width + currentCanvas.bleedings.left + currentCanvas.bleedings.right,
                height: currentCanvas.baseSize.height + currentCanvas.bleedings.top + currentCanvas.bleedings.bottom
            };
            currentCanvas.oriWidth = currentCanvas.baseSize.width;
            currentCanvas.oriHeight = currentCanvas.baseSize.height;
    ...
    getForeground : function(frameBaseSize, expendSize,photoLayer){
        ...
        changeBorderToMirror: function(isWrap) {
            var currentCanvas = Store.pages[Store.selectedPageIdx].canvas;
            var photoLayer = ProjectManage.getPhotoLayer();
            ...
            if(currentCanvas.params.length===1 && currentCanvas.params[0].elType==="image"){
                ...
                if(isWrap){
                    if(currentCanvas.params[0].width==photoLayer.width || currentCanvas.params[0].width==photoLayer.width-2*left){
                        currentCanvas.params[0].width = photoLayer.width;
                        currentCanvas.params[0].x=0;
                    }
                    if(currentCanvas.params[0].height==photoLayer.height || currentCanvas.params[0].height==photoLayer.height-2*top){
                    ...
                    if(currentCanvas.params[0].height==photoLayer.height || currentCanvas.params[0].height==photoLayer.width-2*top){
