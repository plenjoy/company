...
module.exports = {
  template: '<div class="box-handle" id="handles-{{pageIdx}}-{{ id }}" v-bind:style="{ opacity: isShowHandles? 1: 0 }" title="{{ handleTitle }}">' +
              '<!-- center layer -->' +
    ...
    handleTitle: function() {
      ...
      if(this.id !== 'bg') {
        ...
        if(currentCanvas.params[idx].elType === 'image' && currentCanvas.params[idx].imageId === '') {
          return 'Click to add your photo(s)';
        };
      };
  ...
  methods: {
    bindHandleEvents: function(id,pageIdx) {
      this.bindLayerEvents(id,pageIdx);

      if(this.isCornerHandles) {
        this.bindCornerEvents(id,pageIdx);
      };

      if(this.isSideHandles) {
        this.bindSideEvents(id,pageIdx);
      };

    },

    bindLayerEvents: function(id,pageIdx) {
      var _this = this;
      if(id != null) {
        var handle = document.getElementById('handles-' + pageIdx +"-" + id),
    ...
    bindCornerEvents: function(id,pageIdx) {
    ...
    bindSideEvents: function(id,pageIdx) {
      var pageIdx = pageIdx || Store.selectedPageIdx;
      if(id != null) {
        var _this = this;
        var currentCanvas = Store.pages[pageIdx].canvas;
  ...
  ready: function() {
    this.bindHandleEvents(this.id,this.pageIdx);
  }
};
