module.exports = {
	template:
				'<div v-if="!sharedStore.isPortal && !sharedStore.isPreview && oriPrice && sharedStore.isTopPriceShow" v-bind:style="{zIndex:windowZindex}" style="position:absolute;right:40px;top:38px;">'+
	      			'<div style="margin-top:15px;height:16px;line-height:16px;">'+

	          			'<img v-show="sharedStore.isShowDetail" src="./assets/img/title-price.svg" style="width:13px;height:13px;margin:1px 20px 0 10px;cursor:pointer;float:right;" v-on:click="showTotalPrice()"/>'+
						'<span class="font-medium" style="font-size:13px;color:#3a3a3a;margin-left:6px;float:right;">USD</span>'+
	          			'<span class="font-medium" v-show="discount" style="font-size:13px;margin-left:20px;color:#3a3a3a;margin-right:0;float:right;">{{"$" + toFixed(totalPrice, 2)}}</span>'+
						...
						'<span v-if="sharedStore.isShowPriceProduct" class="font-medium" style="font-size:13px;color:#3a3a3a;margin-left:8px;float:right;">{{projectDetail}}</span>' +
	      			...
	      			'<div v-show="sharedStore.isTotalPriceShow && typeof oriPrice !==\'undefined\'" v-bind:style="{zIndex:windowZindex}" style="position:absolute;right:0px;top:43px;width:180px;box-sizing:border-box;border:1px solid #d6d6d6;box-shadow:0 3px 6px rgba(0,0,0,.16);background:#fff;">'+
	      				...
	      				'<div style="margin-top:15px;height:15px;line-height:15px;">'+
		      				'<span class="price-item font-light" style="width:80px;">Prints:</span>'+
						...
						'<div v-for="priceOption in priceOptionList" style="margin-top:5px;height:15px;line-height:15px;">'+
	      				...
	      				// '</div>'+
	      				'<div style="margin-top:5px;height:15px;line-height:15px;width:180px;">'+
	...
	computed : {
		projectDetail: function() {
			...
			var paper = require('SpecManage').getOptionNameById('paper', prj.paper);
			var size = require('SpecManage').getOptionNameById('size', prj.size);
			return size + ', ' + paper + ',';
		},
		...
		preMessage: function() {
			var prj = this.sharedStore.projectSettings[Store.currentSelectProjectIndex];
			if(prj && prj.product === 'LSC' && !this.discount) {
				return '10 cards, '
			} else {
				return ''
			}
