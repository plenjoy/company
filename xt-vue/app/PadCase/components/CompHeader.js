...
module.exports = {
    ...
    template: '<div class="bed-header" v-on:click="blurFocus">' +
                '<div style="display: inline-block; height: 38px;">' +
                  '<div id="logo" v-on:click="handleLogoClicked()" style="margin: 12px 20px 12px 30px;float: left;cursor: pointer;" title="Home">' +
                    '<img src="../PhoneCase/assets/img/new-logo.svg" height="15" width="90" draggable="false" alt="Logo" style="display:block;" />' +
                  '</div>' +
                ...
                '<div style="float: right; height: 38px;">' +
                  ...
                  '<span class="menu-item" v-on:click="handlePreview()" style="height: 38px;line-height: 38px;margin-right: 50px;">Preview</span>' +
                  '<span v-show="!sharedStore.fromCart" id="saveButton" class="menu-item" v-on:click="handleSave()" style="height: 38px;line-height: 38px;margin-right: 50px;">Save</span>' +
                  '<span id="orderButton" class="menu-item" v-on:click="handleOrder()" style="height: 38px;line-height: 38px;margin-right: 30px;">{{orderLabel}}</span>' +
                '</div>' +
    ...
    methods: {
        ...
        //     window.open("/prod-assets/static/spec/poster/"+prj.size+"SPEC_"+prj.product+".zip", "_blank");
        // },
        ...
        handleOrder: function() {
            ...
            if(!this.checkImageExist()) {
                this.$dispatch("dispatchShowPopup", { type : 'orderBlankProductToCart' });
            } else {
                this.$dispatch("notifyOrderToCart");
            }
        },
