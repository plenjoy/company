...
module.exports = {
	...
	template: '<div class="bed-image-upload" v-show="sharedStore.isSingleImageUploadShow">' +
							...
							'<div class="box-single-image-upload" v-bind:style="{ zIndex: windowZindex }">' +
								...
								'<div style="margin-top: 20px;">' +
									...
									'<input type="file" name="" id="single-files" accept="image/jpeg" v-on:change="handleDoUpload()" style="display: none;" />' +
								'</div>' +
							'</div>' +
						'</div>',
	data: function() {
	...
	methods: {
		...
		initImageUploadDom: function(idx, sFileName) {
			...
			if(idx < 5) {
				// use old inited dom
				$('#single-upload-row-item-' + idx + ' .upload-status-head').text(displayFileName).attr('title', sFileName);
				...
				var uploadStatusCont =	'<div class="upload-row-item" id="single-upload-row-item-'+ idx +'">' +
																	'<span class="upload-status-head" title="'+ sFileName +'">'+ displayFileName +'</span>' +
	...
	events: {
		...
		notifyShowSingleImageUpload: function() {
			Store.oriImageIds=[];
