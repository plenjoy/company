	...
	template:
				'<div v-if="!sharedStore.isPortal && !sharedStore.isPreview && oriPrice" v-bind:style="{zIndex:windowZindex}" style="position:absolute;right:40px;top:50px;">'+
	      			'<div style="margin-top:15px;height:16px;line-height:16px;">'+
	          			...
	          			'<span class="font-medium" v-show="discount" style="font-size:14px;color:#3a3a3a;margin-right:0;float:right;">{{"$" + toFixed(totalPrice, 2)}}</span>'+
	          			'<span class="font-normal" style="font-size:14px;color:#3a3a3a;margin-right:20px;float:right;" v-bind:style={textDecoration:priceStyle} >{{"$" + toFixed(totalOriprice, 2)}}</span>'+
