  ...
  template:'<div class="bed-title">'+
              ...
              '<div v-show="privateStore.isEditingTitle">'+
                  '<div class="box-title-edit">'+
                      '<input class="title-edit" type="text" v-model="privateStore.title" v-on:keyUp="handleInputKeyEvent"/>' +
                  ...
                  '</div>'+
                  '<span v-show="privateStore.isTitleInvalid" class="title-warn">{{privateStore.InvalidText}}</span>'+
              '</div>' +
            '</div>',
  data:function() {
    return {
  ...
  methods:{
    ...
    handleTitleEditDone: function() {
      ...
      if (!title.trim()) {
        ...
        //  旧的 title 修改接口。
        // require('ProjectController').addOrUpdateAlbum(this.privateStore.title,this,'dispatchUpdateAlbumResponse');
        //  新的 title 修改接口
