...
module.exports = {
  ...
  mixins: [
	...
	methods: {
		...
		handleOndrop: function(obj) {
			...
			if(obj) {
				...
				if(newAdded) {
					// adding new element
					this.replaceImage(imageId,currentCanvas.elements.length-1);
				}
				else if(isBg) {
					this.replaceImage(imageId,0);
				}
				else {
					idx = parseInt(ev.target.id.split('-')[1]);
					this.$dispatch("dispatchShowPopup", { type: 'replace', status: -1 });
					store.dropData.idx = idx;
				};
			};
		},

		createImage: function(ev) {
			var _this = this,
					store = _this.sharedStore,
					currentCanvas = store.pages[store.selectedPageIdx].canvas;
		...
		replaceImage: function(imageId,idx) {
			var _this = this,
					store = _this.sharedStore,
					currentCanvas = store.pages[store.selectedPageIdx].canvas;
				if(store.shouldNewImage){
					this.createImage(store.dropData.ev);
					store.shouldNewImage = false;
				}
        ...
        var qs = UtilProject.getQueryString({
          ...
          height: Math.round(height)
				...
				$('#element-' + idx).attr('href', '/imgservice/op/crop?' + qs;
				// $.ajax({
				...
				currentCanvas.elements[idx].sourceImageUrl = store.dragData.sourceImageUrl;

				ImageListManage.freshImageUsedCount();
				_this.freshImageList();
				if(currentCanvas.warns[idx] && currentCanvas.warns[idx].el){
						WarnController.deleteElement(idx);
						currentCanvas.warns[idx].isActive = false;
				}
       	CanvasController.initWarnTip(idx);
		},
	...
	events: {
		...
		notifyReplaceImage : function(param){
			this.replaceImage(param.imageId,param.idx);
		},
