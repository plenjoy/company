...
module.exports = {
    ...
    deleteElement: function(idx, elementId) {
        var currentCanvas = Store.pages[Store.selectedPageIdx].canvas;
    ...
    initXMLProject: function(sXml) {
        var _this = this;
        if (sXml) {
            for (var k = 0; k < $(sXml).find('frameBoard').length; k++) {
                // var prj = Store.projectSettings[k];
                // var specData = SpecController.analyseSpec({ size: prj.size, product: prj.product});

                Store.projects[k] = {};
                Store.projects[k].pages = [];
                for (var i = 0; i < $(sXml).find('frameBoard').eq(k).find('elements').length; i++) {
                    Store.projects[k].pages.push({
                        type: $(sXml).find('frameBoard').eq(k).find('elements').eq(i).attr('type') || '',
                        name: '',
                        canvas: {
                            // isInited: false,
                    ...
                    // front
                    if (Store.projectSettings[Store.currentSelectProjectIndex].product === "canvas") {
                      // canvas
                      currentCanvas.realBleedings = ProjectManage.getBleed();
                      currentCanvas.canvasBordeThickness = ProjectManage.getCanvasBorderThickness();
                      ...
                      currentCanvas.foreground = _this.getForeground(currentCanvas.frameBaseSize, {left:0,top:0,right:0,bottom:0},currentCanvas.photoLayer);
                    ...
                    else if(Store.projectSettings[Store.currentSelectProjectIndex].product === "mountPrint" || Store.projectSettings[Store.currentSelectProjectIndex].product === "acrylicPrint" || Store.projectSettings[Store.currentSelectProjectIndex].product === "metalPrint" || Store.projectSettings[Store.currentSelectProjectIndex].product === "woodPrint") {
                      // new print
                      currentCanvas.realBleedings = ProjectManage.getBleed();
                      currentCanvas.frameBaseSize = ProjectManage.getFrameBaseSize();
                      currentCanvas.photoLayer = ProjectManage.getPhotoLayer();
                      currentCanvas.expendSize = { top: 0, right: 0, bottom: 0, left: 0 };
                      currentCanvas.foreground = _this.getForeground(currentCanvas.frameBaseSize, currentCanvas.expendSize,currentCanvas.photoLayer);
                      currentCanvas.oriWidth = currentCanvas.photoLayer.width;
                      currentCanvas.oriHeight = currentCanvas.photoLayer.height;
                      currentCanvas.oriBgWidth = currentCanvas.foreground.width;
                      currentCanvas.oriBgHeight = currentCanvas.foreground.height;
                      var foreground = _this.getForegroundVariable();
                      currentCanvas.canvasBordeThickness = ProjectManage.getCanvasBorderThickness();
                      currentCanvas.oriX = foreground.left * currentCanvas.foreground.ratioX - currentCanvas.canvasBordeThickness.left  - currentCanvas.realBleedings.left;
                      currentCanvas.oriY = foreground.top * currentCanvas.foreground.ratioY - currentCanvas.canvasBordeThickness.top - currentCanvas.realBleedings.top;
                    }
                    else {
                        // frame
                        ...
                        if(Store.projectSettings[Store.currentSelectProjectIndex].product === "frameCanvas"){
                            var foreground = _this.getForegroundVariable();
                            currentCanvas.oriX =  foreground.left * currentCanvas.foreground.ratioX + currentCanvas.frameBorderThickness.left - currentCanvas.canvasBordeThickness.left - currentCanvas.realBleedings.left;
                            currentCanvas.oriY = foreground.top * currentCanvas.foreground.ratioY+ currentCanvas.frameBorderThickness.top - currentCanvas.canvasBordeThickness.top - currentCanvas.realBleedings.top;
                        }
                    }
                    ...
                    var paramsCount = $(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').length;
                    for (var j = 0; j < paramsCount; j++) {
                        var imgId = $(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('imageid') || '',
                        ...
                        var ox = parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('x')) || 0,
                            oy = parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('y')) || 0,
                            ow = parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('width')) || 0,
                            oh = parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('height')) || 0;
                        ...
                        var px=parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('px')) || 0;
                        var py=parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('py')) || 0;
                        var pw=parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('pw')) || 0;
                        var ph=parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('ph')) || 0;
                        ...
                        var elType = '';
                        if ($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('type') === 'PhotoElement') {
                            elType = 'image';
                        } else if ($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('type') === 'TextElement') {
                            elType = 'text';
                        };
                        ...
                        currentCanvas.params.push({
                            ...
                            url: '',
                            isRefresh: false,
                            text: decodeURIComponent($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).text()) || '',
                            ...
                            height: oh,
                            rotate: parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('rot')),
                            dep: parseInt($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('dep')),
                            imageId: imgId,
                            imageGuid: imageDetail.guid || '',
                            imageWidth: imageDetail.width || '',
                            imageHeight: imageDetail.height || '',
                            imageRotate: parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('imgRot')) || 0,
                            // imageFlip: ,
                            cropPX: parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('cropLUX')) || 0,
                            cropPY: parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('cropLUY')) || 0,
                            cropPW: (parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('cropRLX')) - parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('cropLUX'))) || 1,
                            cropPH: (parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('cropRLY')) - parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('cropLUY'))) || 1,
                            fontFamily: decodeURIComponent($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('fontFamily')) || '',
                            fontSize: currentCanvas.oriHeight * parseFloat($(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('fontSize') || 0),
                            fontWeight: $(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('fontWeight') || '',
                            textAlign: $(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('textAlign') || '',
                            fontColor: $(sXml).find('frameBoard').eq(k).find('elements').eq(i).find('element').eq(j).attr('color') || ''
                        });
                    };
    ...
    initJsonProject: function(sJson) {
        ...
        if (sJson) {
            ...
            for (var k = 0; k < pages.length; k++) {
                ...
                var paramsCount = pages[k].elements.length;
                var elements = pages[k].elements;
                for (var j = 0; j < paramsCount; j++) {
                    var imgId = elements[j].imageid || '',
                        imageDetail = ImageListManage.getImageDetail(imgId),
                        sourceImageUrl = '';

                    imageDetail !== '' ? sourceImageUrl = Store.domains.uploadUrl + '/upload/UploadServer/PicRender?qaulityLevel=0&puid=' + imageDetail.encImgId + '&rendersize=fit' : sourceImageUrl;
                    var ox = parseFloat(elements[j].x) || 0,
                        oy = parseFloat(elements[j].y) || 0,
                        ow = parseFloat(elements[j].width) || 0,
                        oh = parseFloat(elements[j].height) || 0;
                   
                    var px=parseFloat(elements[j].px) || 0;
                    var py=parseFloat(elements[j].py) || 0;
                    var pw=parseFloat(elements[j].pw) || 0;
                    var ph=parseFloat(elements[j].ph) || 0;
                    ox=px*currentCanvas.photoLayer.width;
                    oy=py*currentCanvas.photoLayer.height;
                    ow=pw*currentCanvas.photoLayer.width;
                    oh=ph*currentCanvas.photoLayer.height;
                    var elType = '';
                    if (elements[j].type === 'PhotoElement') {
                        elType = 'image';
                    } else if (elements[j].type === 'TextElement') {
                        elType = 'text';
                    };
                    ...
                    currentCanvas.params.push({
                        ...
                        url: '',
                        isRefresh: false,
                        text: decodeURIComponent(elements[j].text) || '',
                        x: ox,
                        y: oy,
                        width: ow,
                        height: oh,
                        rotate: parseFloat(elements[j].rot),
                        dep: parseInt(elements[j].dep),
                        imageId: imgId,
                        imageGuid: imageDetail.guid || '',
                        imageWidth: imageDetail.width || '',
                        imageHeight: imageDetail.height || '',
                        imageRotate: parseFloat(elements[j].imgRot) || 0,
                        cropPX: parseFloat(elements[j].cropLUX) || 0,
                        cropPY: parseFloat(elements[j].cropLUY) || 0,
                        cropPW: parseFloat(elements[j].cropRLX) - parseFloat(elements[j].cropLUX) || 1,
                        cropPH: parseFloat(elements[j].cropRLY) - parseFloat(elements[j].cropLUY) || 1,
                        fontFamily: decodeURIComponent(elements[j].fontFamily) || '',
                        fontSize: currentCanvas.oriHeight * parseFloat(elements[j].fontSize) || 0,
                        fontWeight: elements[j].fontWeight || '',
                        textAlign: elements[j].textAlign || '',
                        fontColor: elements[j].color || ''
                    });
                };
    ...
    freshPageData: function() {
        ...
        if (Store.projectSettings[Store.currentSelectProjectIndex].product === "canvas") {
          ...
          currentCanvas.oriY = foreground.top * currentCanvas.foreground.ratioY - currentCanvas.canvasBordeThickness.top - currentCanvas.realBleedings.top;;
            ...
            if(Store.projectSettings[Store.currentSelectProjectIndex].product === "frameCanvas"){
                var foreground = _this.getForegroundVariable();
                currentCanvas.oriX =  Math.ceil(foreground.left * currentCanvas.foreground.ratioX) + currentCanvas.frameBorderThickness.left - currentCanvas.canvasBordeThickness.left - currentCanvas.realBleedings.left;
                currentCanvas.oriY = Math.ceil(foreground.top * currentCanvas.foreground.ratioY)+ currentCanvas.frameBorderThickness.top - currentCanvas.canvasBordeThickness.top - currentCanvas.realBleedings.top;
            }
        }
    },
    autoLayout : function(){
    ...
    getForegroundVariable : function(){
        var type = 'foreground',
                SpecManage = require("SpecManage"),
    ...
    getForeground : function(frameBaseSize, expendSize,photoLayer){
           var foreground = this.getForegroundVariable();
            if(foreground){
                var ratioX = (frameBaseSize.width + expendSize.left + expendSize.right) / (foreground.width - foreground.left - foreground.right),
                    realBgWidth =  ratioX * foreground.width,
                    ratioY = (frameBaseSize.height + expendSize.top + expendSize.bottom) / (foreground.height - foreground.top - foreground.bottom),
                    realBgHeight = ratioY * foreground.height,
                    realX = photoLayer.x + expendSize.left + foreground.left * ratioX,
                    realY = photoLayer.y + expendSize.top + foreground.top * ratioY;
                return {
                    width: realBgWidth,
                    height: realBgHeight,
                    x : realX,
